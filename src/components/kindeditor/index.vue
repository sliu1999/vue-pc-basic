<template>
	<div class="margin-top-20">
		...
		<input id="inputFile" style="visibility: hidden;height:0;" type="file" name @change="selectedFile" />
	</div>
</template>
<script>
...
import infomationApi from '@/api/person/tech-service';
export default {
	...
	methods: {
		...
		async selectedFile($event) {
			...
			const res = await infomationApi.upload(formData);
			this.$emit('func', res.data.path);
			this.editor.appendHtml(`<img style="" src="${this.$fileUrl}${res.data.path}">`);
		},
		// 编辑器内容上传到cos，调用返回url
		async content2Url() {
